...
for %%p in (Win32,x64) do (
	for %%c in (Release,Release_SSE2,Debug) do (
		MSBuild /property:Platform=%%p /property:Configuration=%%c /target:Clean   "%~dp0\pipe-utils.sln"
		if not "!ERRORLEVEL!"=="0" goto:BuildHasFailed
		MSBuild /property:Platform=%%p /property:Configuration=%%c /target:Rebuild "%~dp0\pipe-utils.sln"
		MSBuild /property:Platform=%%p /property:Configuration=%%c /target:Build   "%~dp0\pipe-utils.sln"
		if not "!ERRORLEVEL!"=="0" goto:BuildHasFailed
	)
)
